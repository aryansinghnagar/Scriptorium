...
def package_codex_bundle(target_path: Path, output_dir: Path) -> dict:
    """Creates World Lore Codex ZIP with static HTML codex, maps, and assets."""
    output_dir.mkdir(parents=True, exist_ok=True)
    title = target_path.name.replace("_", " ")
    zip_name = f"{target_path.name}_Codex_Bundle.zip"
    zip_path = output_dir / zip_name

    exports_dir = target_path / "Exports"
    codex_dir = target_path / "Codex"
    maps_dir = target_path / "Maps"
    art_dir = target_path / "03-Art"

    files_added = []
    with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as z:
        for search_dir, subfolder in [
            (exports_dir, "Codex_Exports"),
            (codex_dir, "Static_Codex"),
            (maps_dir, "Maps"),
            (art_dir, "Art"),
        ]:
            if search_dir.is_dir():
                for f in search_dir.rglob("*"):
                    if f.is_file() and f.suffix.lower() in (".html", ".svg", ".png", ".jpg", ".css", ".js", ".json"):
                        rel = f.relative_to(search_dir)
                        z.write(f, arcname=f"{subfolder}/{rel}")
                        files_added.append(f.name)

        readme = f"# {title} — World Lore Codex Bundle\n\nGenerated by Ars Arcanum on {datetime.date.today().isoformat()}.\nIncludes offline static lore wiki, interactive maps, and worldbuilding assets.\n"
        z.writestr("CODEX_INFO.txt", readme)
        files_added.append("CODEX_INFO.txt")
...
def main():
    ...
    parser.add_argument("target", help="Manuscript or World Vault directory path")
    parser.add_argument("-t", "--type", choices=["reader", "submission", "arc", "codex", "all"], default="all", help="Package target type (default: all)")
    ...
    if args.type == "codex":
        c_pkg = package_codex_bundle(target_path, out_dir)
        packages.append(c_pkg)
    elif args.type == "all":
        # Only auto-include codex if codex or map files exist in target
        if any((target_path / d).is_dir() for d in ("Codex", "Maps")):
            c_pkg = package_codex_bundle(target_path, out_dir)
            packages.append(c_pkg)
