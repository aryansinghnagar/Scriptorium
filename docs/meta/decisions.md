...
- **Context**: A line-by-line external audit of commit `9d34fae` (report committed as `docs/audits/2026-09-16-external-audit.md`; 22 findings — 1 High, 7 Medium, 11 Low, 3 Info) confirmed the engineering discipline but identified one high-impact functional bug (the Calibre Flathub ID `com.calibredesk.calibre` does not exist, so setup could never install Calibre), several injection/fail-open weaknesses (world-doctor JSON interpolated into `python3 -c` source strings, verification stages built on `set -e` + `cmd && echo` that were structurally unable to fail, a Typst installer that failed open on missing digests), identity leakage (the developer's username hardcoded as a heuristic in four scripts), and roughly 300 lines of copy-pasted world-discovery logic across five scripts with partial copies in five more.
- **Decision**:
...
- **Context**: An independent re-audit of the remediation series (record committed as `docs/audits/2026-09-17-remediation-re-audit.md`) verified all 22 original findings closed — grade A (9.0/10), zero regressions — and registered five new observations: N-01 the CONTRIBUTING exit-code contract no longer matched the code, N-02 the remediation commits carried a container identity instead of the maintainer's, N-03 the legacy `~/Worlds` deprecation nudge missed the auto-select path, N-04 non-daemon worker threads plus GUI-thread `flatpak info` probes in the GTK app, and N-05 a dead legacy Calibre ID in the launch chain. Separately, the repository root had accumulated six planning documents and the audit report alongside the standard OSS entry files, which the re-audit's hygiene review noted as reader-facing clutter.
- **Decision**:
  - Document the exit-code contract as the real four-value semantics (0 success, 1 runtime/diagnostic failure, 2 usage/environment error, 3 nothing to act on), with each script's header remaining the authoritative per-script contract (N-01).
  - Extract the legacy-root message into `warn_if_legacy_root` in `scripts/lib/worlds.sh` and invoke it from every auto-select site, so bare invocations that land on a legacy world are nudged exactly like by-name resolutions (N-03).
  - Centralize thread creation in `_start_worker` (always `daemon=True`) and dispatch the five external-app launch chains — with their `flatpak info` / `which` probes — onto daemon workers via `_launch_in_background`; replace the `date` subprocess in snapshot defaults with `datetime.now()` (N-04).
  - Delete the never-matching `com.calibredesk.calibre` fallback branch (N-05).
  - Declutter the root to the standard OSS entry set (README, CHANGELOG, LICENSE, CONTRIBUTING, SECURITY): the six planning/meta documents move to `docs/meta/`, audit reports move to date-stamped files under `docs/audits/`, a `CHANGELOG.md` is added, and all cross-links are updated.
  - Reset authorship of the entire remediation series (everything after `9d34fae`) to the maintainer's GitHub identity (N-02), accepting that publishing the rewritten history requires `git push --force-with-lease origin main` rather than a fast-forward push.
- **Consequences**: The documented contract matches the code; the deprecation nudge is consistent across every selection path; quitting the app mid-operation no longer leaves a lingering process; launch clicks no longer stutter the UI; the root contains only files every contributor expects at a glance; and the public history reads as the maintainer's own remediation work while the full audit trail — including the re-audit that graded it — remains committed for transparency.
