...
| **Metadata Menu** | Structured Frontmatter & Schemas | Strict `fileClass` schemas in `Templates/fileClasses/` with dropdown validation for character status, location scales, faction influence, and timeline years. |
...
2. **Metadata Validation**:
   - `Templates/fileClasses/` provides pre-built schemas for:
     - `Character.md`: Name, aliases, role, status (Alive/Deceased/Missing), faction, location.
     ...
     - `TimelineEvent.md`: Name, era, start_year, end_year, location, participants.
...
3. **Manuscript & Maps Folders**:
   ...
   - `03-Art/` serves as the default attachment folder for visual character and location assets.
